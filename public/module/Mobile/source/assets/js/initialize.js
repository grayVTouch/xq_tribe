...
G.ajax.opened = function(res){
    ...
    // 用户登录认证头部
    if (G.s.exists('app.user')) {
        let user = G.s.json('app.user');
        if (user.login === 'logged') {
            this.setRequestHeader('token' , user.token);
        }
    }
};
...
// 设置根元素字体
G.setRootFontSize(750 , 100);
topContext.win.on('resize' , G.setRootFontSize.bind(G , 750 , 100));